...
def data_process_sam_seg(path_dataset, dataset_name, data_type, patch_size, model_type):
    # data_type: train or test

    input_image_path = path_dataset + "/raw/" + data_type + "/images/"
    input_gt_path = path_dataset + "/raw/" + data_type + "/gt/"
    save_path = (
        path_dataset + "/" + model_type + "/" + str(patch_size) + "/" + data_type + "/"
    )
    ...
    rm = [".ipynb_checkpoints", ".DS_Store", ".xml"]
    input_label = [input for input in input_label if input not in rm]
    ...
    for g_id, label in enumerate(tqdm(input_label)):
        ...
        # read data
        label_name = label.split("_gt")[0]
        print(input_image_path)
        path_img = os.path.join(input_image_path, f"{label_name}.tif")
        print(path_img)
        ...
        path_gt = os.path.join(input_gt_path, label)
        ...
        for i in range(len(patch_gt_list)):
            ...
            io.imsave(
                os.path.join(output_gt_train, p_name), p_gt_3ch, check_contrast=False
            )

            train_im_id += 1


def data_process_sam_seg_final(path_database, dataset_name, data_list, data_type, patch_size):
    for dataset in data_list:
        path_dataset = os.path.join(path_database, dataset)
        print("Start processing: " + dataset + " " + data_type + " by patch size " + str(patch_size))

        data_process_sam_seg(path_dataset, dataset_name, data_type, patch_size, model_name)
...
def manipulate_image(input_path, output_path, operation="rotate", degrees=90):
    ...
    if operation == "rotate":
        ...
        raise ValueError(
            "Operation can only be 'rotate', 'horizontal_flip', or 'vertical_flip'"
        )
...
def augmentation_single(data_list, operation, degrees, op_times, path_base):
...
def data_process_augmentation_final(
    path_database,
    data_list,
    type_list,
    patch_size,
    ...
    for dataset in data_list:
        for dtype in type_list:
            ...
            path_base = os.path.join(
                path_database, dataset, model_name, str(patch_size)
            )
            ...
            for operation in operation_list:
                ...
                if operation != "rotate":
                    ...
                    print(
                        operation
                        + " is not included in the codes. Please try horizontal_flip, vertical_flip, rotate"
                    )
    ...
    for dataset in data_sr_list:
        for dtype in type_sr_list:
            ...
            path_dataset = os.path.join(
                path_database, dataset, model_name, str(patch_size), dtype, "images"
            )
...
def upscale_testing_image_by_SR(path_in_img, sr_model, factor=4, patch_size=512):
    ...
    print(
        "num_patches_height: {}, num_patches_width: {}".format(
        ...
        for w_idx in range(num_patches_width):
            ...
            if h_idx < num_patches_height - 1:
                ...
                if w_idx < num_patches_width - 1:
                    new_array[start_h_new:end_h_new, start_w_new:end_w_new, :] = (
                        upscaled_patch
                    )
                else:
...
def upscale_test_cubic(path_in_img, path_out_img, upscale_factor):
    ...
    with rasterio.open(path_in_img) as dataset:
        ...
        data = dataset.read(
            out_shape=(dataset.count, height, width), resampling=resampling
        )
        print(data.shape)
        ...
        print(f"saved upscaled test image to path {path_out_img}")


def upscale_testing_data_cubic_final(path_database="", dataset="", data_type="", patch_size=256, test_file_path=""):
    ...
    if test_file_path:
        path_out_img_ = test_file_path.replace(".tif", "_upscaled.tif")
        upscale_test_cubic(test_file_path, path_out_img_, upscale_factor)
        return True
    ...
    for in_img in in_img_list:
        ...
        path_in_img = os.path.join(path_test_img, in_img)
        print("Processing by {}: {}".format(upscaled_folder, path_in_img))
        upscale_test_cubic(path_in_img, path_out_img, upscale_factor)
